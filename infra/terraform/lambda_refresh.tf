...
resource "aws_lambda_function" "sentiment_refresh" {
  ...
  role          = aws_iam_role.sentiment_refresh_lambda.arn
  handler       = "handler.lambda_handler"
  runtime       = "python3.12"

  filename         = data.archive_file.sentiment_refresh_lambda.output_path
  source_code_hash = data.archive_file.sentiment_refresh_lambda.output_base64sha256
